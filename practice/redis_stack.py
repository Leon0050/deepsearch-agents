...
def _check_redis():
    ...
    try:
        r = redis_stack.Redis.from_url(REDIS_URL, decode_responses=True)
        r.ping()
        r.close()
    except (redis_stack.ConnectionError, redis_stack.ResponseError) as e:
        logger.error(
...
redis_client = redis_stack.Redis.from_url(REDIS_URL, decode_responses=True)
logger.info(
